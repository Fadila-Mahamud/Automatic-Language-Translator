...
resource "aws_s3_bucket" "request_bucket" {
  bucket = "fadila-translate-requests"
  force_destroy = true
}

# Response bucket (output JSON)
resource "aws_s3_bucket" "response_bucket" {
  bucket = "fadila-translate-responses"
  force_destroy = true
}

# IAM Role for Lambda
resource "aws_iam_role" "lambda_role" {
  name = "fadila-translate-lambda-role"
...
resource "aws_lambda_function" "translate_lambda" {
  function_name = "fadila-translate-lambda"
